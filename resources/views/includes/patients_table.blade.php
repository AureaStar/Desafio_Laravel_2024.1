...
<x-adminlte-modal id="show{{$cell->id}}" title="Detalhes do {{$title}}" theme="teal" icon="{{$icon}}" size="lg" static-backdrop>
    ...
    <x-adminlte-input name="image" label="Imagem" placeholder="Imagem" value="{{ $cell->patient->image }}" disabled />
...
<x-adminlte-modal id="edit{{$cell->id}}" title="Editar Dados do {{$title}}" theme="blue" icon="{{$icon}}" size="lg" static-backdrop>
    <form action="{{ route($route . '.update', $cell->$type->id) }}" method="POST">
        ...
        <x-adminlte-input type="image" name="image" label="Imagem" placeholder="Imagem" value="{{ $cell->$type->image }}" />
...
<x-adminlte-select name="health_plan" label="Plano de Saúde" placeholder="Plano de Saúde" >
