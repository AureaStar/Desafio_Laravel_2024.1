    ...
    <form class="form-horizontal" action="{{ route('patient.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <x-adminlte-input name="birth_date" label="Data de Nascimento" placeholder="Data de Nascimento" type="date" value="{{ $user->patient->birth_date }}" />
            </div>
            <div class="col-md-6">
                <!-- Campos da segunda coluna -->
                <x-adminlte-input name="phone" label="Telefone" placeholder="(XX) XXXXX-XXXX" value="{{ $user->patient->phone }}" oninput="formatarTelefone(this)" />
                ...
                <x-adminlte-select name="health_plan_id" label="Plano de Saúde" placeholder="Plano de Saúde" value="{{ $user->patient->health_plan !== null ?  $user->patient->health_plan->id : '' }}" class="select2">
                    @foreach ($health_plans as $health_plan)
                        <option value="{{ $health_plan->id }}"  {{ $user->patient->health_plan->id == $health_plan->id ? 'selected' : '' }} >{{ $health_plan->name }}</option>
                    @endforeach
                </x-adminlte-select>
                <x-adminlte-select name="blood_type" label="Tipo Sanguíneo" placeholder="Tipo Sanguíneo" value="{{ $user->patient->blood_type !== null ? $user->patient->blood_type : '' }}" class="select2">
